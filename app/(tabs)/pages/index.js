...
const Pages = () => {
	...
	const getPosts = async () => {
		...
		try {
			const options = {
				method: "GET",
				url: `${WP_SITE_URL}/wp-json/wp/v2/pages`,
				params: {
					per_page: 10,
			...
			setPages((prevPosts) => [...prevPosts, ...response.data]);
		...
		<SafeAreaView style={{ flex: 1, backgroundColor: COLORS.lightWhite }}>
			...
			<FlatList
				...
				renderItem={({ item }) => (
					<PostCard
						item={item}
						handleCardPress={() => router.push({ pathname : '/blog-page', params: { postType: 'pages', id: item.id } })}
					/>
				)}
